-- Drop and recreate get_services_with_creators function to include creator tier
DROP FUNCTION IF EXISTS public.get_services_with_creators(text, text, numeric, numeric, text);
...
CREATE OR REPLACE FUNCTION public.get_services_with_creators(
  ...
  sort_option text DEFAULT 'newest'::text
)
RETURNS TABLE(
  service_id uuid, 
  title text, 
